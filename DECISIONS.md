...
### 2.13 One ask and one follow-up per game, enforced by the database

**Decision.** A unique index on `notification (game_id, kind)` for
`score_request` and `nudge`, rather than reading back what was already sent.

**Why.** The scheduler runs every minute and overlaps with itself, and the
Saturday evening burst is exactly when two runs will race. A read-then-write
check has a window; an index does not. Nothing is more corrosive to a
volunteer's willingness to reply than a robot asking six times.

**Consequence.** Re-asking means requeueing the existing row, not inserting a
new one — which is what the retry on `/hq/messages` does, and why it can take a
corrected phone number.

---

### 2.14 An ask is abandoned if the score arrives first

**Decision.** The queue drain re-checks, at send time, whether a
`score_request` or `nudge` is still wanted, and abandons it if a report or an
approved score has landed since.

**Why.** Messages can sit in the queue — a long code paces at one per second,
and a provider outage can back things up for an hour. A "reply with the score"
arriving long after the game was phoned in to HQ is not merely useless; it
teaches volunteers that the system does not know what is going on. There is
also a hard expiry (two hours) for the case where the queue stops entirely.

**Consequence.** `abandoned` is a distinct status from `failed`, so the
failures screen can separate "the send broke" from "we stopped on purpose".

---

### 2.15 Coaches are never asked for their own game's score

**Decision.** When a game's diamond has nobody on shift, the ask is not
rerouted to the two coaches. The gap is reported instead.

**Why.** "Coaches are unreliable reporters" is a stated pain point (§3), and
asking a coach to report on their own game invites precisely the bias the
diamond volunteer exists to avoid. A coach texting unprompted is still accepted
— that is path 2, and it goes through the same approval queue.

**Consequence.** Uncovered games surface on `/hq/diamonds`, in the cron
response, and in the server log. A staffing gap is a staffing problem and gets
put in front of a person as one.

---

### 2.16 Every outbound template is plain ASCII

**Decision.** No em dashes, curly quotes or accents in anything the system
texts. Enforced by a test over every template.

**Why.** SMS is billed per segment. A message inside the GSM-7 alphabet gets
160 characters per segment; one character outside it drops the whole message to
UCS-2 and 70 characters. The inbound replies already had em dashes and were
quietly billing double on the longer ones — nobody would have noticed until the
Twilio bill arrived in August. Running costs come out of donation dollars (§11).

**Consequence.** Message bodies moved out of SQL `format()` into
`src/domain/messaging.ts`, so one place decides what a coach reads and the
tests can guard it.

---

### 2.17 A failure reading an inbound text parks it, rather than returning 500

**Decision.** If parsing throws — the model call times out, the database blips
— the message is written to the unmatched screen with reason
`processing_error` and the sender gets a normal acknowledgement.

**Why.** The fallback chain ends at "a human at HQ types it" (§4). Returning a
500 would make Twilio retry into the same failure several times and then give
up, and the volunteer's report would be gone. Parking it costs one row and
keeps the guarantee.

---

### 2.18 The transport refuses to pretend

**Decision.** In production, missing Twilio credentials are a hard failure, not
a silent fall back to logging. Running deliberately without sending requires
`ALLOW_LOG_SMS=true`, and the log transport writes an obviously fake `log:`
provider id to the database.

**Why.** A row marked `sent` that never went anywhere is the worst outcome
available here, and the kind that stays hidden until Saturday evening with
ninety coaches waiting. The dashboard has to be able to lie only in the safe
direction.
...
| SMS broadcast composer and the rain button | §5.7 — the transport underneath both is built and tested; neither has a screen yet. The rain button should be preview-then-confirm, never a single button that fires. |
| Quiet hours on outbound | Games run in daylight and messages are generated by game events, so the two-hour expiry on asks covers the realistic failure (a provider outage draining at 3am) without another setting to get wrong. |
...
0. **Which sending number.** A long code paces at roughly one message per
   second, so a broadcast to ninety teams takes about three minutes to drain
   and Saturday evening will have bursts overlapping. A Messaging Service with
   a toll-free number fixes it for a monthly fee. Not a code change —
   `SMS_RATE_PER_SECOND` already takes whatever the account is provisioned for.
...
5. **Cell coverage at each diamond.** No offline queueing is built. If coverage
   is bad anywhere, path 1 needs a fallback there specifically — the ask will
   send and simply never arrive, which looks identical to a volunteer ignoring
   it.
